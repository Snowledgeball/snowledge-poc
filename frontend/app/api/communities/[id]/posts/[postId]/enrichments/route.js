...
export async function POST(request, { params }) {
  try {
    ...
    const { content, original_content, title, description } =
      await request.json();
    ...
    const userIds = contributors
      .filter(
        (contributor) => contributor.user.id !== parseInt(session.user.id)
      )
    ...
    const communityCreator = await prisma.community.findUnique({
      where: {
    ...
    await createBulkNotifications({
      userIds: [communityCreator.creator_id],
      type: NotificationType.NEW_ENRICHMENT_PENDING,
      title: "Nouvelle contribution",
      message: `Un nouvel enrichissement a été soumis pour le post "${post.title}"`,
      link: `/community/${communityId}/posts/${postId}/enrichments/${contribution.id}/review`,
