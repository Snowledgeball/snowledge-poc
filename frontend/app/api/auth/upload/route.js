...
export async function POST(request) {
  try {
    ...
    const name = (userName + "'s" + " DID").trim();
    const description = "Decentralized Identity of " + userName;
...
export async function PUT(request) {
  ...
  const contract = new Contract(abiSBT, addressSBT, funderAccount);

  try {
    // TODO: change to the user address
    const userAddress = data.get("userAddress");
    console.log("userAddress :", userAddress);
    const txResponse = await contract.call("get_token_uri_by_address", [
      userAddress,
    ]);
    ...
    if (newValue.community && file.community) {
      ...
      if (
        !Array.isArray(file.community) &&
        file.community.name === newValue.community.name
      ) {
        ...
        if (
          Array.isArray(file.community) &&
          file.community.find(
            (community) => community.name === newValue.community.name
          )
        ) {
          file.community.find(
            (community) => community.name === newValue.community.name
        ...
        //Si file.community n'est pas un tableau (au moins 1 communauté deja ajoutée) et que la newValue.community.name n'existe pas dans file.community, on remplace la valeur de role
          ...
          const existingCommunities = Array.isArray(file.community)
            ? file.community
            : [file.community];
    ...
    //Si y a un champs profilePicture, on le supprime de newValue
    if (file.profilePicture) {
      console.log("profilePicture field exists");
      delete newValue.profilePicture;
    }
    ...
    const estimatedFee = await contract.estimate("set_token_uri_by_address", [
      userAddress,
      newMetadataUrl,
    ]);
    const txResponseSetUri = await contract.invoke(
      "set_token_uri_by_address",
      [userAddress, newMetadataUrl],
      {
