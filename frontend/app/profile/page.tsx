...
const ProfilePage = () => {
  ...
  const [
    ...
    setSelectedCommunityForContribution,
  ...
  const [selectedCommunity, setSelectedCommunity] =
    useState<UserCommunity | null>(null);
  ...
  const fetchContentProposals = useCallback(async () => {
    ...
    try {
      ...
      if (
        profileCache.contentProposals &&
        profileCache.contentProposals.has(cacheKey)
      ) {
      ...
      if (!profileCache.contentProposals) {
        profileCache.contentProposals = new Map<
          string,
          { data: any[]; timestamp: number }
        >();
      }
  ...
  const calculateTotalContributions = useCallback(() => {
    ...
    const totalContributions =
      enrichmentsCount + reviewsCount + contributorRequestsCount;
  ...
  useEffect(() => {
    ...
    if (
      ...
      setUserData((prevData) => {
        ...
        const totalContributions =
          enrichments.length + reviews.length + contentProposals.length;
      ...
      const fetchSelectedCommunityPosts = async () => {
        try {
          ...
          const response = await fetch(
            `/api/users/${userId}/posts/${selectedCommunity.id}`,
            {
              headers: {
                "Cache-Control": "max-age=120", // Cache de 2 minutes côté serveur
              },
  ...
  const handleContributorRequest = async () => {
    ...
    try {
      ...
      const community = joinedCommunities.find(
        (c) => c.name === selectedCommunityForContribution
      );
      ...
      const response = await fetch(
        `/api/communities/${community.id}/contributor-requests`,
        {
          method: "POST",
          headers: {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            ...
            <Card
              key={index}
              className="p-6 bg-white hover:bg-gray-50 transition-colors duration-200"
            >
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center ${stat.color} bg-gray-50`}
                >
                          ...
                          <div
                            onClick={() =>
                              setSelectedCommunity(
                                community as unknown as UserCommunity
                              )
                            }
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <div className="flex items-center mt-2 text-sm text-gray-500">
                                <span>
                                  {formatDistanceToNow(
                                    new Date(enrichment.created_at),
                                    {
                                      addSuffix: true,
                                      locale: fr,
                                    }
                            ...
                            <button
                              onClick={() =>
                                router.push(
                                  `/community/${enrichment.community_posts.community.id}/posts/${enrichment.community_posts.id}`
                                )
                              }
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mt-3">
                            ...
                            <button
                              onClick={() =>
                                router.push(
                                  `/community/${selectedCommunity?.id}/posts/${post.id}`
                                )
                              }
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex gap-4">
                            <button
                              onClick={() =>
                                router.push(
                                  `/community/${community.id}/dashboard`
                                )
                              }
