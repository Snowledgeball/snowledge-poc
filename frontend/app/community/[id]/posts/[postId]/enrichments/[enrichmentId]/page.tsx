...
export default function ContributionDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<"diff" | "original" | "modified">(
    "diff"
  );
  ...
  const fetchContribution = async () => {
    try {
      ...
      const response = await fetch(
        `/api/user/enrichments/${params.enrichmentId}`
      );
        ...
        <div className="mb-4">
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="mb-3">{getStatusBadge(contribution.status)}</div>
            </div>
