...
import { generateStarkNetAddress, deployAccountContract } from "../utils/starknetUtils";
...
const GenerateAddress = () => {
    ...
    const handleDeployContract = async () => {
        ...
        try {
            ...
            setDeployStatus(`Contrat déployé avec succès ! Transaction hash : ${result.transactionHash}`);
        } catch (error: unknown) {
            console.log("error", error);
            if (error instanceof Error) {
                setDeployStatus(`Erreur : ${error.message}`);
    ...
    return (
        <div className="max-w-md mx-auto p-6 bg-white rounded-2xl shadow-lg">
            ...
            <p className="text-gray-600 text-center mb-6">
                Cliquez sur le bouton ci-dessous pour générer une adresse, ajoutez des fonds, puis déployez le contrat.
            </p>
            <button
            ...
            {addressDetails && (
                <div className="mt-6 p-4 bg-gray-100 rounded-lg">
                    ...
                    <p className="text-gray-800 font-mono break-all">{addressDetails.accountAddress}</p>
                    <p className="text-sm text-gray-500 mt-4">Clé Publique :</p>
                    <p className="text-gray-800 font-mono break-all">{addressDetails.publicKey}</p>
                    <p className="text-sm text-gray-500 mt-4">Clé Privée :</p>
                    <p className="text-gray-800 font-mono break-all">{addressDetails.privateKey}</p>
                    <button
