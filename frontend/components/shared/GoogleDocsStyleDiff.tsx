  ...
  const advancedDiffHtml = (oldHtml: string, newHtml: string): string => {
    ...
    try {
      ...
      for (const [op, text] of diffs) {
        if (op === -1) {
          ...
          processedText = processedText.replace(
            /\n/g,
            '<span style="display: inline-block; background-color: rgba(255, 204, 204, 0.8); border: 1px solid #f44336; border-radius: 3px; padding: 0 3px; margin: 0 2px; font-size: 0.8em; vertical-align: middle;">↵</span> '
          );
      ...
      result = result.replace(
        /<span class="diff-added"><span class="diff-added">(.*?)<\/span><\/span>/g,
        '<span class="diff-added">$1</span>'
      );
      result = result.replace(
        /<span class="diff-removed"><span class="diff-removed">(.*?)<\/span><\/span>/g,
      ...
      console.error(
        "Erreur lors de la génération des différences avancées:",
        error
      );
