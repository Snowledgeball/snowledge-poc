...
const toUint256 = (value) => {
  ...
  return {
    low: `0x${(
      bigValue & BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF")
    ).toString(16)}`,
...
export const fundAccount = async (recipientAddress, amount) => {
  ...
  try {
    const estimatedFee = await ethContract.estimate("transfer", [
      recipientAddress,
      u256Amount,
    ]);
    console.log("estimatedFeeeeeee transfer", estimatedFee);
    const tx = await ethContract.invoke(
      "transfer",
      [recipientAddress, u256Amount],
      {
    ...
    console.log(
      `Transfert réussi ! Hash de la transaction : ${tx.transaction_hash}`
    );
...
export const deployAccountContract = async (privateKey, publicKey) => {
  ...
  try {
    let estimatedFee;
    try {
      estimatedFee = await account.estimateAccountDeployFee({
        classHash: ACCOUNT_CLASS_HASH,
        constructorCalldata: [publicKey],
        addressSalt: salt,
      });
      console.log("estimatedFee deployAccount", estimatedFee);
    } catch (estimateError) {
      console.warn("Erreur lors de l'estimation des frais:", estimateError);
      estimatedFee = { overall_fee: BigInt("10000000000000000") };
    }

    try {
      await fundAccount(accountAddress, estimatedFee.overall_fee * BigInt(2));
      await waitForFunds(accountAddress, estimatedFee.overall_fee * BigInt(2));
    } catch (error) {
    ...
    console.log(
      `Compte déployé avec succès ! Hash de transaction : ${deployTx.transaction_hash}`
    );
